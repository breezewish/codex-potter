...
//!
//! This policy preserves the baseline user experience while adapting to bursty stream input. In
//! [`ChunkingMode::Smooth`], one queued line is drained per baseline commit tick. When queue
//! pressure rises, it switches to [`ChunkingMode::CatchUp`] and drains queued backlog immediately
//! so display lag converges as quickly as possible.
//!
//! The policy is source-agnostic: it depends only on queue depth and queue age from
//! [`QueueSnapshot`]. It does not branch on source identity or explicit throughput targets.
//!
//! The transition logic intentionally uses hysteresis to avoid rapid mode flapping near threshold
//! boundaries.
